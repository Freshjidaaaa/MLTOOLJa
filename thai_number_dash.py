...
#----------------------------------------------------------------------
    html.H5('Function for Testing Number'),
    html.Div([
        dcc.Dropdown(['Count','Max','Min','Sum', 'Mean','Median','Mode'], id='demo-dropdown'),
        html.Div(id='dd-output-container')
    ]),
    html.Br(),
    html.Button('Reset', id='button-example-2'),
...
def update_output4(value):
    ...
    if value == 'Sum':
        if len(predicted) == 0:
            return f'Please upload image for testing first.'
        total = sum(predicted)
        return html.Div([
                    html.Br(),
                    html.Div(children=[
                        daq.LEDDisplay(
                            value=total,
                            label = "Summation of Testing Data",
                            style={'width': '33%', 'display': 'inline-block'}
                            ),
                    ]),
                ])
        ...
        if len(predicted) == 0:
            return f'Please upload image for testing first.'
        modes = mode(predicted)
        return html.Div([
                    html.Br(),
                    html.H5(f"Mode of Testing Data : {modes}",
                            style={'width': '33%', 'display': 'inline-block'}),
                ])
    elif value == 'Max':
        if len(predicted) == 0:
        ...
        if len(predicted) == 0:
            return f'Please upload image for testing first.'
        med = median(predicted)
        return html.Div([
                    html.Br(),
                    html.Div(children=[
                        daq.LEDDisplay(
                            value=med,
                            label = "Median of Testing Data",
                            style={'width': '33%', 'display': 'inline-block'}
                            ),
                    ]),
                ])
